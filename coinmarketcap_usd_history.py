...
def processDataFrame(df):
  ...
  for col in cols: df.loc[:,col] = df[col].apply(lambda x:  fill_missing_value(x))
...
def main(args=None):
  ...
  final_data_df = processDataFrame(pd.DataFrame(data=rows,columns=header)) 
  ...
  if(args.dataframe):
    ...
    csv_name = currency+".csv" 
    
    final_data_df.to_csv('csv_file\\'+csv_name, encoding = "utf-8", index = False)
